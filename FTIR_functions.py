...
def apodization(interferogram = None, max_index = None, function = None):
    ...
    apodization_functions = ["crop","triangular","happ-genzel","blackmann-harris","forward ramp","backward ramp","high pass","trapizoid"]
    ...
    if "trapizoid" in function:
        ramp_width = int(0.2 * L)
        trapizoid_function = np.ones(L)
        trapizoid_function[:ramp_width] = np.linspace(0, 1, ramp_width)
        trapizoid_function[-ramp_width:] = np.linspace(1, 0, ramp_width)
        interferogram = interferogram * trapizoid_function
    if not set(function) & set(apodization_functions):
        print("WARNING! Apodization was not applied!") # 'no' apodization is equivelent to box/boxcar/tophat apodization.
